...
proc poke:movetemplate:default {move trainer pokedet otrainer opokedet {extra ""}} {
  ...
  if {$extra != ""} {
    foreach {setting value} $extra {
      if {[info exists $setting]} {set $setting [subst $value]}
    }
  }
  if {$dmgtype in {"miss" "no effect"}} {
    if {$move in {"Future Sight" "Doom Desire"}} {set dmgtype "timedfail"}
    poke:message $dmgtype $move $trainer $pokedet $otrainer $opokedet - 0
    return [list $returntype $dmg]
  }
  if {$dmg < 1} {set dmg 1}
  ...
  if {$move in {"Future Sight" "Doom Desire"}} {set dmgtype "timed[string map {{ } {}} $dmgtype]"}
  ...
  for {set i 1} {$i <= $counter} {incr i} {
    ...
    switch $dmgtype {
      ...
      default {
        ...
        if {$move eq "Triple Kick"} {incr bp 10}
      }
    }
  }
  incr i -1
...
proc poke:movetemplate:forceswitch {move trainer pokedet otrainer opokedet {dmg 0} {rand 1}} {
  ...
  if {$test < $acc || $acc eq "-"} {
    if {$rand} {
      ...
      putquick "PRIVMSG $trainer :Please pick a Pokémon (use \"switch pokemonname\" or \"switch pokemonnumber\"): [join $available_list {, }]"
...
proc poke:movetemplate:trap {move trainer pokedet otrainer opokedet {trap 0} {trigger 0}} {
  ...
  if {$trigger} {
    ...
    poke:update_pokemon $otrainer $opokedet status "add trapped $move $turns"
    array set opokemon $opokedet
  ...
  if {!$trap} {
    array set opokemon $opokedet
    if {[lsearch -glob $opokemon(status) "trapped $move *"] > -1} {
    ...
    set faint 0
    lassign [poke:update_pokemon $otrainer $opokedet status "add trapped $move -1"] result $opokedet
    array set opokemon $opokedet
    array set pokemon $pokedet
    set dmg [expr {int($opokemon(HP)*0.0625)}]
    poke:update_pokemon $otrainer $opokedet "cHP" -$dmg
    switch $move {
      "Sand Tomb" {set msg "trapped by the quick sand"}
      "Whirlpool" {set msg "trapped by the vortex"}
    }
    poke:message custom - $trainer $pokedet $otrainer $opokedet - 0 "$msg! It suffered $dmg damage"
    if {[lsearch -glob $opokemon(status) "trapped $move *"] > -1 && $pokemon(species) in $poke(currentPoke)} {
      lappend poke(prio-9) [list $opokemon(Spd) poke:movetemplate:trap $move $trainer $pokedet $otrainer $opokedet 1]
    }
...
proc poke:movetemplate:multiturn {move trainer pokedet otrainer opokedet count} {
  ...
  if {$count == 0} {    
    switch -regexp $move {
      {Thrash|Outrage} {
        poke:message customself - $trainer $pokedet $otrainer $opokedet - 0 "calmed down and became confused"
        poke:update_pokemon $trainer $pokedet status "add conf"
      }
      {Uproar} {
        poke:message customself - $trainer $pokedet $otrainer $opokedet - 0 "calmed down"
        set idx [lsearch -glob $poke(field) "uproar *"]
...
proc poke:move:stomp {trainer pokedet otrainer opokedet} {
  ...
  array set opokemon $opokedet
  set bp 65
  set allUsed [lsearch -inline -all -index 1 -regexp [lreverse $poke(log)] "$otrainer's .+? used .*?!"]
  foreach used $allUsed {
    if {$used eq "" || ($used ne "" && [string first $opokemon(species) $used] < 0)} {
      break
    }
    regexp {\S+?'s .+? used (.+)!} $used - uMove
    if {$uMove eq "Minimize"} {
      set bp 130
      break
    }
  }
  lassign [poke:movetemplate:default "Stomp" $trainer $pokedet $otrainer $opokedet [list bp $bp] faint dmg
  poke:random
  set test [rand 100]
  if {$test < 30} {
...
proc poke:move:disable {trainer pokedet otrainer opokedet} {
  ...
  if {$opokemon(ability) eq "Pressure"} {incr ppdown}
  poke:update_pokemon $trainer $pokedet "Move" "-$ppdown $move"
  if {[lsearch -glob $opokemon(status) "disable*"] > -1} {
    poke:message fail $move $trainer $pokedet $otrainer $opokedet - 0
    return 0
  }
  ...
  regexp {\S+?'s \S+ used (.+)!} $used - uMove
  poke:update_pokemon $otrainer $opokedet status "add disable $uMove 4"
  poke:message use $move $trainer $pokedet $otrainer $opokedet - 0
  poke:message custom - $trainer $pokedet $otrainer $opokedet - 0 "got its $uMove disabled"
  lappend poke(prio-10) [list 0 poke:status disable $otrainer $opokedet $trainer $pokedet]
  return 0
}
...
# 73 Leech Seed
proc poke:move:leechseed {trainer pokedet otrainer opokedet} {
  poke:movetemplate:stats "Leech Seed" $trainer $pokedet $otrainer $opokedet [list seeded] op
  return 0
}
...
# 102 Mimic
proc poke:move:mimic {trainer pokedet otrainer opokedet} {
  global poke
  set move "Mimic"
  array set opokemon $opokedet
  array set pokemon $pokedet
  ...
  regexp {\S+?'s \S+ used (.+)!} $used - uMove
  set mRestriction [lmap m [array get pokemon Move*] {
    if {[lindex $m 1 0] eq $move} {set miD [lindex $m 0]}
    set m [lindex $m 1 0]
  }]
  lappend mRestriction Sketch Struggle Metronome Chatter
  if {$uMove in $mRestriction} {
    poke:message fail $move $trainer $pokedet $otrainer $opokedet - 0
    return 0
  }
  foreach moveDet [array get opokemon Move*] {
    if {[lindex $moveDet 1 0] eq $uMove} {
      set maxPP [lindex $moveDet 1 2]
      break
    }
  }
  poke:update_pokemon $trainer $pokedet $mID "$uMove $maxPP $maxPP"
  poke:message use $move $trainer $pokedet $otrainer $opokedet - 0
  poke:message customself - $trainer $pokedet $otrainer $opokedet - 0 "mimicked its opponent's $uMove"
  return 0
}
...
# 113 Light Screen
proc poke:move:lightscreen {trainer pokedet otrainer opokedet {decr 0}} {
  global poke
  set move "Light Screen"
  if {$decr == 0} {
    set state [lsearch -glob $poke(field) "lightscreen $trainer *"]
    if {$state > -1} {
      lappend poke(field) [list lightscreen $trainer 5]
      lappend poke(prio-9) [list 0 poke:move:lightscreen $trainer $pokedet $otrainer $opokedet 1]
      poke:message use $move $trainer $pokedet $otrainer $opokedet - 0
      poke:message fullcustom - $trainer $pokedet $otrainer $opokedet - 0 "$move made the opposing team stronger against special moves"
    } else {
      poke:message fail $move $trainer $pokedet $otrainer $opokedet - 0
    }
  } else {
    set id [lsearch -glob $poke(field) "lightscreen $trainer *"]
    regexp {lightscreen \S+ (\d)} [lindex $poke(field) $id] - turns
    incr turns -1
    if {$turns > 0} {
      lset poke(field) $id "lightscreen $trainer $turns"
      lappend poke(pending) [list -9 0 poke:move:lightscreen $trainer $pokedet $otrainer $opokedet 1]
    } else {
      array set pokemon $pokedet
      set poke(field) [lreplace $poke(field) $id $id]
      poke:message fullcustom $trainer $pokedet $otrainer $opokedet - 0 "$pokemon(species)'s $move wore off"
    }
  }
  return 0
}
...
# 115 Reflect
proc poke:move:reflect {trainer pokedet otrainer opokedet {decr 0}} {
  global poke
  set move "Reflect"
  if {$decr == 0} {
    set state [lsearch -glob $poke(field) "reflect $trainer *"]
    if {$state > -1} {
      lappend poke(field) [list reflect $trainer 5]
      lappend poke(prio-9) [list 0 poke:move:reflect $trainer $pokedet $otrainer $opokedet 1]
      poke:message use $move $trainer $pokedet $otrainer $opokedet - 0
      poke:message fullcustom - $trainer $pokedet $otrainer $opokedet - 0 "$move made the opposing team stronger against physical moves"
    } else {
      poke:message fail $move $trainer $pokedet $otrainer $opokedet - 0
    }
  } else {
    set id [lsearch -glob $poke(field) "reflect $trainer *"]
    regexp {reflect \S+ (\d)} [lindex $poke(field) $id] - turns
    incr turns -1
    if {$turns > 0} {
      lset poke(field) $id "reflect $trainer $turns"
      lappend poke(pending) [list -9 0 poke:move:reflect $trainer $pokedet $otrainer $opokedet 1]
    } else {
      array set pokemon $pokedet
      set poke(field) [lreplace $poke(field) $id $id]
      poke:message fullcustom $trainer $pokedet $otrainer $opokedet - 0 "$move of $pokemon(species) wore off"
    }
  }
  return 0
}
...
proc poke:move:bide {trainer pokedet otrainer opokedet {state 0} {dmg 0}} {
  ...
  switch -regexp $state {
    ...
    3 {
      set otrainerID [lsearch $poke(currentPoke) $otrainer]
      set opponentPoke [lindex $poke(currentPoke) $otrainer+1]
      
      set opokedet [poke:update_det $otrainerID $opponentPoke]
      array set opokemon $opokedet
      ...
      set dmg [expr {int($dmg*2*ceil($weakness))}]
      
      set faint [poke:movetemplate:default "Bide" $trainer $pokemon $otrainer $opokemon [list dmgtype bide dmg $dmg crit 0]]
      set ppdown 1
      if {$opokemon(ability) eq "Pressure"} {incr ppdown}
      poke:update_pokemon $trainer $pokedet "Move" "+$ppdown Bide"
      return $faint
...
# 119 Mirror Move
proc poke:move:mirrormove {trainer pokedet otrainer opokedet} {
  global poke
  set move "Mirror Move"
  array set opokemon $opokedet
  array set pokemon $pokedet
  poke:trigger launch $otrainer $opokedet $trainer $pokedet
  set ppdown 1
  ...
  foreach used $allUsed {
    ...
    regexp {\S+?'s \S+ used (.+)!} $used - uMove
    set movedet [dex eval "SELECT * FROM $table WHERE name = '$uMove'"]
    if {[lindex $movedet 15] == 1} {break}
  }
  poke:message use $move $trainer $pokedet $otrainer $opokedet - 0
  poke:message customself - $trainer $pokedet $otrainer $opokedet - 0 "mirrored its opponent's $uMove"
  set faint [poke:move:[string tolower [join $uMove ""]] $trainer $pokedet $otrainer $opokedet]
  return $faint
}
...
# 144 Transform
proc poke:move:transform {trainer pokedet otrainer opokedet} {
  global poke
  set move "Transform"
  array set opokemon $opokedet
  array set pokemon $pokedet
  ...
  poke:update_pokemon $trainer $pokedet species $opokemon(species)
  poke:update_pokemon $trainer $pokedet ability $opokemon(ability)
  poke:update_pokemon $trainer $pokedet type $opokemon(type)
  
  lassign {0 0 0 0 0 0 0} Atk Def SpA SpD Spd Eva Acc
  foreach stat $opokemon(status) {
    switch [lindex $stat] {
      Atk {set Atk [expr {int($opokemon(Atk)*[poke:boost $opokemon(status) "Atk"])}]}
      Def {set Def [expr {int($opokemon(Def)*[poke:boost $opokemon(status) "Def"])}]}
      SpA {set SpA [expr {int($opokemon(SpA)*[poke:boost $opokemon(status) "SpA"])}]}
      SpD {set SpD [expr {int($opokemon(SpD)*[poke:boost $opokemon(status) "SpD"])}]}
      Spd {set Spd [expr {int($opokemon(Spd)*[poke:boost $opokemon(status) "Spd"])}]}
      Eva {set Eva [expr {int($opokemon(Eva)*[poke:boost $opokemon(status) "Eva"])}]}
      Acc {set Acc [expr {int($opokemon(Acc)*[poke:boost $opokemon(status) "Acc"])}]}
    }
  }
  switch $pokemon(item) {
    "Choice Band"  {set Atk [expr {int($Atk*1.5)}]}
    "Choice Specs" {set SpA [expr {int($SpA*1.5)}]}
    "Choice Scarf" {set Spd [expr {int($Spd*1.5)}]}
  }
  
  poke:update_pokemon $trainer $pokedet Atk $Atk
  poke:update_pokemon $trainer $pokedet Def $Def
  poke:update_pokemon $trainer $pokedet SpA $SpA
  poke:update_pokemon $trainer $pokedet SpD $SpD
  poke:update_pokemon $trainer $pokedet Spd $Spd
  poke:update_pokemon $trainer $pokedet Eva $Eva
  poke:update_pokemon $trainer $pokedet Acc $Acc
  
  poke:update_pokemon $trainer $pokedet Move1 [lreplace $opokemon(Move1) 1 1 5]
  poke:update_pokemon $trainer $pokedet Move2 [lreplace $opokemon(Move2) 1 1 5]
  poke:update_pokemon $trainer $pokedet Move3 [lreplace $opokemon(Move3) 1 1 5]
  poke:update_pokemon $trainer $pokedet Move4 [lreplace $opokemon(Move4) 1 1 5]
  
  poke:message use $move $trainer $pokedet $otrainer $opokedet - 0
  poke:message customself - $trainer $pokedet $otrainer $opokedet - 0 "transformed into $opokemon(species)"
  return 0
}
...
# 164 Substitute
proc poke:move:substitute {trainer pokedet otrainer opokedet {subs 1}} {
  global poke
  array set pokemon $pokedet
  if {$subs} {
    set move "Substitute"
    set price [expr {int(ceil($pokemon(HP)*0.25))}]
    if {$pokemon(cHP) < $price || [lsearch -index 0 $pokemon(status) "substitute"] > -1} {
      poke:message fail $move $trainer $pokedet $otrainer $opokedet
    } else {
      poke:update_pokemon $trainer $pokedet "Move" "-1 $move"
      poke:update_pokemon $trainer $pokedet "cHP" "-$price"
      poke:update_pokemon $trainer $pokedet status "add substitute $price"
      lappend poke(triggers) [list launch poke:move:substitute $otrainer $opokedet $trainer $pokedet 0]
      poke:message use $move $trainer $pokedet $otrainer $opokedet - 0
      poke:message customself - $trainer $pokedet $otrainer $opokedet - 0 "created a substitute"
    }
    return 0
  } else {
    unset trainer pokedet otrainer opokedet
    upvar 2 trainer trainer pokedet pokedet otrainer otrainer opokedet opokedet bp bp acc acc type type class class flags flags extra extra move move counter counter
    lassign $flags - - - - - - - sound - - - - infiltrate
    set movetype ""
    for {set level [info level]} {$level > 0} {incr level -1} {
      if {[regexp "poke:movetemplate:" [info level $level]]} {set movetype [lindex [info level $level] 0]}
      if {[regexp "poke:move:$move" [info level $level]]} {break}
    }
    array set pokemon $pokedet
    if {$pokemon(ability) eq "Infiltrator" || $sound == 1 || $infiltrate == 1} {
      return
    } elseif {$bp == "-"} {
      poke:message fail $move $trainer $pokedet $otrainer $opokedet - 0
    ...
    switch $movetype {
      poke:movetemplate:default {
        ...
        if {$extra != ""} {
          foreach {setting value} $extra {
            if {[info exists $setting]} {
              ...
              if {$setting eq "dmg" && $dmgtype ni {miss "no effect"}} {
                lassign {normal 0} dmgtype crit
              }
            }
          }
        }
        if {$dmgtype in {"miss" "no effect"}} {
          poke:message $dmgtype $move $trainer $pokedet $otrainer $opokedet - 0
          return -level [expr {[info level]-$level+1}] 0 $dmg
        }
        if {$dmg < 1} {set dmg 1}
        set sID [lsearch -index 0 $opokemon(status) "substitute"]
        set cHP [lindex $opokemon(status) $sID 1]
        set hp [expr {$cHP-$dmg}]
        poke:message use $move $trainer $pokedet $otrainer $opokedet - 0
        poke:message fullcustom - $trainer $pokedet $otrainer $opokedet - 0 "The substitute took the damage"
        if {$hp <= 0} {
          set triggerID [lsearch -glob $poke(triggers) "* poke:move:substitute *"]
          set poke(triggers) [lreplace $poke(triggers) $triggerID $triggerID]
          poke:update_pokemon $otrainer $opokedet status [lreplace $opokemon(status) $sID $sID]
          poke:message fullcustom - $trainer $pokedet $otrainer $opokedet - 0 "The substitute broke"
        } else {
          poke:update_pokemon $otrainer $opokedet status [lreplace $opokemon(status) $sID $sID "substitute $hp"]
        }
      }
      poke:movetemplate:multi {
        poke:message use $move $trainer $pokedet $otrainer $opokedet - 0
        for {set i 1} {$i <= $counter} {incr i} {
          poke:trigger "mid launch" $otrainer $opokedet $trainer $pokedet
          lassign [poke:damage_calc $pokedet $opokedet $bp $acc $type $class $flags] dmgtype dmg crit
...
# 166 Sketch
proc poke:move:sketch {trainer pokedet otrainer opokedet} {
  global poke
  set move "Sketch"
  array set opokemon $opokedet
  array set pokemon $pokedet
  ...
  lappend mRestriction Struggle Chatter
  if {$uMove in $mRestriction} {
    poke:message fail $move $trainer $pokedet $otrainer $opokedet - 0
    return 0
  }
  ...
  poke:update_pokemon $trainer $pokedet $mID "$uMove $maxPP $maxPP"
  poke:update_pokemon $trainer $pokedet o$mID "$uMove $maxPP $maxPP"
  poke:message use $move $trainer $pokedet $otrainer $opokedet - 0
  poke:message customself - $trainer $pokedet $otrainer $opokedet - 0 "learned $uMove"
  return 0
}
...
proc poke:move:triplekick {trainer pokedet otrainer opokedet} {
  global poke
  set faint [poke:movetemplate:multi "Triple Kick" $trainer $pokedet $otrainer $opokedet 3]
  return $faint
}
...
proc poke:move:thief {trainer pokedet otrainer opokedet} {
  ...
  if {$pokemon(item) eq "" && $opokemon(item) ne "" && $opokemon(ability) ne "Sticky Hold" && [lsearch -index 0 $opokemon(status) substitute] == -1} {
...
# 169 Spider Web
proc poke:move:spiderweb {trainer pokedet otrainer opokedet} {
  poke:update_pokemon $otrainer $opokedet status "add blocked"
  poke:message use "Spider Web" $trainer $pokedet $otrainer $opokedet - 0
  poke:message custom - $trainer $pokedet $otrainer $opokedet - 0 "cannot flee anymore"
  return 0
}

# 170 Mind Reader
proc poke:move:mindreader {trainer pokedet otrainer opokedet} {
  global poke
  array set pokemon $pokedet
  array set opokemon $opokedet
  if {[lsearch $pokemon(status) "locked $opokemon(species)"] > -1} {
    poke:update_pokemon $trainer $pokedet status "add locked $opokemon(species)"
    poke:message use "Mind Reader" $trainer $pokedet $otrainer $opokedet - 0
    poke:message customself - $trainer $pokedet $otrainer $opokedet - 0 "locked on its opponent"
    lappend poke(pending) [list -10 0 poke:update_pokemon $trainer $pokedet status "rem locked"]
  } else {
    poke:message fail "Mind Reader" $trainer $pokedet $otrainer $opokedet - 0
  }
  return 0
}

# 171 Nightmare
proc poke:move:nightmare {trainer pokedet otrainer opokedet} {
  array set opokemon $opokedet
  if {[lsearch -index 0 $opokemon(status) "SLP"] > -1 && [lsearch -index 0 $opokemon(status) "nightmare"] == -1} {
    poke:update_pokemon $otrainer $opokedet status "add nightmare"
    poke:message use "Nightmare" $trainer $pokedet $otrainer $opokedet - 0
    poke:message custom - $trainer $pokedet $otrainer $opokedet - 0 "is afflicted with Nightmare"
  } else {
    poke:message fail "Nightmare" $trainer $pokedet $otrainer $opokedet - 0
  }
  return 0
}
...
# 174 Curse
proc poke:move:curse {trainer pokedet otrainer opokedet} {
  global poke
  array set pokemon $pokedet
  if {[string match "*Ghost*" $pokemon(type)]} {
    set cost [expr {$pokemon(HP)/2}]
    poke:update_update $trainer $pokedet cHP -$cost
    poke:update_update $otrainer $opokedet status "add curse"
    poke:message use "Curse" $trainer $pokedet $otrainer $opokedet - 0
    poke:message customself - $trainer $pokedet $otrainer $opokedet - 0 "inflicted damage to itself and cursed its opponent"
    lappend poke(prio-9) [list 0 poke:status curse $otrainer $opokedet $trainer $opokedet]
  } else {
    poke:movetemplate:stats "Curse" $trainer $pokedet $otrainer $opokedet [list "Atk +1" "Def +1 Spd -1"] self
  }
  return 0
}
...
# 176 Conversion 2
proc poke:move:conversion2 {trainer pokedet otrainer opokedet} {
  global poke
  set table moveDetails$poke(gen)
  array set opokemon $opokedet
  set allUsed [lsearch -inline -all -index 1 -regexp [lreverse $poke(log)] "$otrainer's .+? used .*?!"]
  set type "None"
  set move "Conversion 2"
  set ppdown 1
  if {$opokemon(ability) eq "Pressure"} {incr ppdown}
  poke:update_pokemon $trainer $pokedet "Move" "-$ppdown $move"
  foreach used $allUsed {
    ...
    regexp {\S+?'s .+? used (.+)!} $used - uMove
    set uMove [string map {' ''} $uMove]
    lassign [dex eval "SELECT basepower, type FROM $table WHERE name = '$uMove'"] dmg type
    if {$dmg ne "-"} {
      break
    }
  }
  if {$type eq "None"} {
    ...
    array set pokemon $pokedet
    set types [dex eval "
      SELECT type1 FROM matcDetails$poke(gen)
      WHERE effectiveness < 1 AND type2 = '$type' AND type1 != '$pokemon(type)'
    "]
    if {$types eq ""} {
      poke:message fail $move $trainer $pokedet $otrainer $opokedet - 0
    } else {
      ...
      poke:random
      set target [rand $len]
      poke:update_pokemon $trainer $pokedet type [lindex $types $target]
      poke:message use $move $trainer $pokedet $otrainer $opokedet - 0 ""
      poke:message customself - $trainer $pokedet $otrainer $opokedet - 0 "changed types!"
    }
  }
  return 0
}
...
# 180 Spite
proc poke:move:spite {trainer pokedet otrainer opokedet} {
  global poke
  array set opokemon $opokedet
  set used [lsearch -inline -index 1 -regexp [lreverse $poke(log)] "$otrainer's .+? used .*?!"]
  set ppdown 1
  if {$opokemon(ability) eq "Pressure"} {incr ppdown}
  poke:update_pokemon $trainer $pokedet "Move" "-$ppdown Spite"
  poke:trigger launch $otrainer $opokedet $trainer $pokedet
  if {$used eq "" || ($used ne "" && [string first $opokemon(species) $used] < 0)} {
    poke:message fail "Spite" $trainer $pokedet $otrainer $opokedet - 0
  } else {
    regexp {\S+?'s .+? used (.+)!} $used - uMove
    foreach {moveno movedet} [array get opokemon "Move*"] {
      if {[lindex $movedet 0] eq $uMove} {
        set curPP [lindex $movedet 1]
        break
      }
    }
    if {$curPP == 0} {
      set msg "But there was no PP left for the move"
    } else {
      set opokedet [poke:update_pokemon $otrainer $opokedet "Move" "-4 $uMove"]
      array set opokemon $opokedet
      set finalPP [lindex $opokemon($moveno) 1]
      set diff [expr {$curPP-$finalPP}]
      set msg "It reduced the PP of $opokemon(species)'s $uMove by $diff"
    }
    poke:message use "Spite" $trainer $pokedet $otrainer $opokedet - 0
    poke:message fullcustom - $trainer $pokedet $otrainer $opokedet - 0 $msg
  }
  return 0
}
...
# 182 Protect
proc poke:move:protect {trainer pokedet otrainer opokedet {trigger 0}} {
  global poke
  if {$trigger} {
    for {set level [info level]} {$level > 0} {incr level -1} {
      if {[regexp {poke:move:} [info level $level]] && [info level $level] ne "poke:move:protect"} {break}
    }
    
    poke:message customself - $trainer $pokedet $otrainer $opokedet - 0 "protected itself"
    return -level [expr {[info level]-$level+1}] 0
  } else {
    poke:update_pokemon $trainer $pokedet "Move" "-1 $move"
    lappend poke(triggers) [list launch poke:move:protect $trainer $pokedet $otrainer $opokedet 1]
    poke:message use "Protect" $trainer $pokedet $otrainer $opokedet - 0
    poke:message customself - $trainer $pokedet $otrainer $opokedet - 0 "protected itself"
  }
  return 0
}
...
# 199 Lock-On
proc poke:move:lock-on {trainer pokedet otrainer opokedet} {
  global poke
  array set pokemon $pokedet
  array set opokemon $opokedet
  if {[lsearch $pokemon(status) "locked $opokemon(species)"] > -1} {
    poke:update_pokemon $trainer $pokedet status "add locked $opokemon(species)"
    poke:message use "Lock-On" $trainer $pokedet $otrainer $opokedet - 0
    poke:message customself - $trainer $pokedet $otrainer $opokedet - 0 "locked on its opponent"
    lappend poke(pending) [list -10 0 poke:update_pokemon $trainer $pokedet status "rem locked"]
  } else {
    poke:message fail "Lock-On" $trainer $pokedet $otrainer $opokedet - 0
  }
  return 0
}
...
# 210 Fury Cutter
proc poke:move:furycutter {trainer pokedet otrainer opokedet} {
  array set pokemon $opokedet
  set prev [lsearch -inline -index 1 -regexp [lreverse $poke(log)] "$trainer's \\S+? used .*?!"]
  set bp 40
  set level 1
  if {$prev ne "" && [string first $opokemon(species) $used] > -1 && [string first miss $used] == -1} {
    regexp {\S+?'s \S+ used (.+)!} $prev - uMove
    if {$uMove eq "Fury Cutter"} {
      set furyLevel [lsearch -inline -glob $pokemon(status) "fury *"]
      lassign [split $furyLevel] - level
      if {$level < 6} {incr level}
      set bp [expr {$bp*(2**$level)}]
    }
  }
  lassign [poke:movetemplate:default "furycutter" $trainer $pokedet $otrainer $opokedet [list bp $bp]] faint dmg
  poke:update_pokemon $trainer $pokedet status "fury $level"
  return $faint
}
...
# 212 Mean Look
proc poke:move:meanlook {trainer pokedet otrainer opokedet} {
  poke:update_pokemon $otrainer $opokedet status "add blocked"
  poke:message use "Mean Look" $trainer $pokedet $otrainer $opokedet - 0
  poke:message custom - $trainer $pokedet $otrainer $opokedet - 0 "cannot flee anymore"
  return 0
}
...
proc poke:move:rapidspin {trainer pokedet otrainer opokedet} {
  ...
  array set pokemon $pokedet
  for {set i 0} {$i < [llength $pokemon(status)} {} {
    if {[string match "*trapped*" $stat] || [string match "*seeded*" $stat]} {
      poke:update_pokemon $trainer $pokedet status "rem [lindex $pokemon(status) $i]"
    } else {
      incr i
    }
  }
  # Remove spikes, toxic spikes, stealth rock, sticky web, etc
  return $faint
}
...
proc poke:move:futuresight {trainer pokedet otrainer opokedet {charged 0}} {
  ...
  if {$charged == 2} {
    ...
    set ids [list]
    set i 0
    foreach trigger $poke(triggers) {
      if {[lindex $trigger 0] eq "launch"} {
        lappend triggers $trigger
        lappend ids $i
      }
      incr i
    }
    foreach id [lreverse $ids] {
      set poke(triggers) [lreplace $poke(triggers) $id $id]
    } 
    set faint [poke:movetemplate:default $trainer $pokedet $otrainer $opokedet]
    lappend poke(triggers) {*}$triggers
    ...
    set pokeID [lsearch -regexp [lindex $poke(team) $trainerID 1] "\\yspecies \\{?$pokemon(species)\\y\\}?"]
    set opponentID [lsearch $poke(currentPoke) $otrainer]
    if {[lsearch -glob $poke(prio-9) "*poke:move:futuresight $trainer*"] > -1} {
      poke:message fail $move $trainer $pokedet $otrainer $opokedet - 0
      return 0
    }
    if {$charged == 0} {
...
# 253 Uproar
proc poke:move:uproar {trainer pokedet otrainer opokedet} {
  poke:random
  set count [expr {[rand 2]+2}]
  lappend poke(field) "uproar $trainer"
  return [poke:movetemplate:multiturn "Uproar" $trainer $pokedet $otrainer $opokedet $count]
}
...
proc poke:move:knockoff {trainer pokedet otrainer opokedet} {
  ...
  if {$opokemon(item) ne "" && $opokemon(ability) ne "Sticky Hold" && [lsearch -index 0 $opokemon(status) substitute] == -1} {
    foreach a [split $opokemon(item) ""] b [split $opokemon(species) ""] {
      if {$a eq $b} {incr match} else {break}
    }
    if {($opokemon(ability) eq "Multitype" && [string match {*Plate} $opokemon(item)]) || 
      ...
      lassign [poke:movetemplate:default "Knock Off" $trainer $pokedet $otrainer $opokedet [list dmg {[expr {int($dmg*1.5)}]}]] faint dmg
      array set pokemon [poke:update_det [lsearch -index 0 $poke(team) $trainer] $pokemon(species)]
      if {$pokemon(cHP) == 0} {return $faint}
      poke:update_pokemon $otrainer $opokedet item ""
      poke:update_pokemon $otrainer $opokedet oitem ""
      poke:message customself - $trainer $pokedet $otrainer $opokedet - 0 "stole $opokemon(item) from its opponent"
    }
...
# 335 Block
proc poke:move:block {trainer pokedet otrainer opokedet} {
  poke:update_pokemon $otrainer $opokedet status "add blocked"
  poke:message use "Block" $trainer $pokedet $otrainer $opokedet - 0
  poke:message custom - $trainer $pokedet $otrainer $opokedet - 0 "cannot flee anymore"
  return 0
}
...
proc poke:move:doomdesire {trainer pokedet otrainer opokedet {charged 0}} {
  ...
  if {$charged == 2} {
    ...
    incr charged
    lappend poke(pending) [list -9 $pokemon(Spd) "poke:move:doomdesire" $trainer $pokemon(species) $trainerID $pokeID $opponentID $charged]
  }
}
...
proc poke:move:happyhour {trainer pokedet otrainer opokedet} {
  ...
  poke:message fullcustom - $trainer $pokedet $otrainer $opokedet - 0 "Everyone is caught up in the happy atmosphere"
  return 0
}
...
# 615 Thousand waves
proc poke:move:thousandwaves {trainer pokedet otrainer opokedet} {
  lassign [poke:movetemplate:default "Thousand Waves" $trainer $pokedet $otrainer $opokedet] faint dmg
  poke:update_pokemon $otrainer $opokedet status "add blocked"
  poke:message use "Spider Web" $trainer $pokedet $otrainer $opokedet - 0
  poke:message custom - $trainer $pokedet $otrainer $opokedet - 0 "cannot flee anymore"
  return 0
